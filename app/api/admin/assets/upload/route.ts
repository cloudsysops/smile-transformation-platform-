...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (uploadError) {
      log.error("Failed to upload asset file", { error: uploadError.message, storage_path: storagePath });
    ...
    if (error) {
      log.error("Failed to insert uploaded asset metadata", { error: error.message, storage_path: storagePath });
      return internalServerError(requestId);
    }

    return NextResponse.json(data, { status: 201 });
  } catch (error) {
    log.error("Unhandled asset upload error", { error: getErrorMessage(error) });
    return internalServerError(requestId);
  }
}
